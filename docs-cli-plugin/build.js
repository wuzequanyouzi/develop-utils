...
function loadBuildConfig(api, modules) {
  // 打包配置
...
function loadBuildLogMdConfig(api) {
  // 编译CHANGELOG.md
...
module.exports = (api) => {
  ...
  return (...args) => {
    ...
    if (mode === "docs") {
      ...
      loadBuildConfig(api, modules);
      loadBuildLogMdConfig(api);
      console.log(`👌 ${bold(`加载配置成功`)}`);
      

      const p = buildCallBack(...args);
      p.then(() => {
        const compsPathMap = generateCompsPath(docsConfig);
        console.log(compsPathMap);
        let metaList = [];
        for (let index = 0; index < modules.length; index++) {
          const module = modules[index];
        ...
        const changelogComPath = getChangelogCompPath(docsConfig);
        if (projectIndex === -1) {
          projects.push({
            name: projectInfo.name,
            version: projectInfo.version,
            components: metaList,
            changelog: changelogComPath,
        ...
        console.log(targetChangelogPath, 'targetChangelogPath');
